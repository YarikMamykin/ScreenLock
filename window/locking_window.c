...
const char *lockscreen(struct locking_window *lw, int screen) {
  ...
  lw->locks[screen]->win = XCreateWindow(
      lw->dpy, lw->locks[screen]->root, 0, 0, XDisplayWidth(lw->dpy, screen),
      XDisplayHeight(lw->dpy, screen), 0, XDefaultDepth(lw->dpy, screen),
      CopyFromParent, DefaultVisual(lw->dpy, screen),
  ...
  lw->locks[screen]->pmap =
      XCreateBitmapFromData(lw->dpy, lw->locks[screen]->win, curs, 8, 8);
  Cursor invisible =
      XCreatePixmapCursor(lw->dpy, lw->locks[screen]->pmap,
                          lw->locks[screen]->pmap, &color, &color, 0, 0);
  XDefineCursor(lw->dpy, lw->locks[screen]->win, invisible);

  if (GrabSuccess !=
      XGrabPointer(lw->dpy, lw->locks[screen]->root, False,
                   ButtonPressMask | ButtonReleaseMask | PointerMotionMask,
                   GrabModeAsync, GrabModeAsync, None, None, CurrentTime)) {
    return "Error grabbing mouse";
  }

  if (XGrabKeyboard(lw->dpy, lw->locks[screen]->root, True, GrabModeAsync,
...
void draw_input_stars(struct locking_window *lw,
                      struct password_input_handler *pih, unsigned long color) {
  ...
  for (int i = 0; i < lw->nscreens; ++i) {
    ...
    const int text_width = XTextWidth(
        lw->font_info, (const char *)input_to_display, pih->inserted_chars);
    ...
    XDrawString(lw->dpy, lw->locks[i]->win, gc,
                screen_width / 2 - text_width / 2,
                screen_height / 2 - text_height / 2,
                (const char *)input_to_display, pih->inserted_chars);
  }
}

void draw_input(struct locking_window *lw, struct password_input_handler *pih,
                unsigned long color) {
  ...
  for (int i = 0; i < lw->nscreens; ++i) {
    ...
    const int text_width = XTextWidth(lw->font_info, (const char *)pih->input,
                                      pih->inserted_chars);
    const int text_height = lw->font_info->ascent + lw->font_info->descent;
    const int screen_width = XDisplayWidth(lw->dpy, i);
    const int screen_height = XDisplayHeight(lw->dpy, i);
    ...
    XDrawString(lw->dpy, lw->locks[i]->win, gc,
                screen_width / 2 - text_width / 2,
                screen_height / 2 - text_height / 2, (const char *)pih->input,
                pih->inserted_chars);
  }
}

void draw_start_message(struct locking_window *lw, struct user_data *ud,
                        unsigned long color) {
  ...
  for (int i = 0; i < lw->nscreens; ++i) {
    ...
    const char *start_message = ud->no_password ? "Press any key to unlock..."
                                                : "Waiting for user input...";

    const int text_width =
        XTextWidth(lw->font_info, start_message, strlen(start_message));
    const int text_height = lw->font_info->ascent + lw->font_info->descent;
    const int screen_width = XDisplayWidth(lw->dpy, i);
    const int screen_height = XDisplayHeight(lw->dpy, i);
    ...
    XDrawString(lw->dpy, lw->locks[i]->win, gc,
                screen_width / 2 - text_width / 2,
                screen_height / 2 - text_height / 2, start_message,
...
void draw_greeting(struct locking_window *lw, const char *uname,
                   unsigned long color) {
  ...
  for (int i = 0; i < lw->nscreens; ++i) {
    ...
    const unsigned int greeting_msg_size =
        strlen(greeting_msg_base) + strlen(uname) + strlen(greeting_msg_end);
    ...
    const int text_width =
        XTextWidth(lw->font_info, (const char *)greeting_msg, greeting_msg_len);
    const int text_height = lw->font_info->ascent + lw->font_info->descent;
    const int screen_width = XDisplayWidth(lw->dpy, i);
    const int screen_height = XDisplayHeight(lw->dpy, i);
    ...
    XDrawString(lw->dpy, lw->locks[i]->win, gc,
                screen_width / 2 - text_width / 2,
                screen_height / 2 - text_height / 2, (const char *)greeting_msg,
                greeting_msg_len);
  }
}

void draw_input_info(struct locking_window *lw,
                     struct password_input_handler *pih, unsigned long color) {
...
const char *run_ui(struct user_data *ud) {
  ...
  struct locking_window *lw =
      (struct locking_window *)calloc(1, sizeof(struct locking_window));
  lw->font_info = NULL;
